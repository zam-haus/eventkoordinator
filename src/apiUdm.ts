...
export type FormElementIn = components['schemas']['FormElementIn']
export type FormElementOut = components['schemas']['FormElementOut']
export type FormElementBindingIn = components['schemas']['FormElementBindingIn']
export type FormElementBindingOut = components['schemas']['FormElementBindingOut']
...
export async function udmListEntitiesByType(typeId: string, pageSize = 200, q = ''): Promise<EntityOut[]> {
  const query = q ? `&q=${encodeURIComponent(q)}` : ''
  const resp = await fetch(
    `/api/udm/entities/?type_id=${encodeURIComponent(typeId)}&page_size=${pageSize}${query}`,
    { credentials: 'include' },
  )
