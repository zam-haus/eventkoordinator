package udm.udmframeworkv1.modules.description

import data.udm.udmframeworkv1.modules.config._deadline
import data.udm.udmframeworkv1.modules.config._deadline_human
import rego.v1

# ── Deadline ──────────────────────────────────────────────────────────────────

_days_left := round((time.parse_rfc3339_ns(_deadline) - time.now_ns()) / (((24 * 60) * 60) * 1000000000))

# ── Pill ──────────────────────────────────────────────────────────────────────

_pill_color := "red" if _days_left < 0

_pill_color := "yellow" if {
	_days_left >= 0
	_days_left <= 7
}
...
_pill_text := concat("", [sprintf("%v", [0 - _days_left]), " days overdue"]) if _days_left < 0
